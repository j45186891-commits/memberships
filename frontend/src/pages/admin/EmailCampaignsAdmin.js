...
function EmailCampaignsAdmin() {
  ...
  const handleSave = async (formData) => {
    try {
      if (selectedItem) {
        await api.put(`/email-campaigns/${selectedItem.id}`, formData);
  ...
  const handleBulkDelete = async (ids) => {
    try {
      await Promise.all(ids.map((id) => api.delete(`/email-campaigns/${id}`)));
  ...
  const handleExport = async (selectedIds) => {
    try {
      ...
      a.href = url;
      a.download = `email-campaigns_${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
      ...
      <AdminFormDialog
        ...
        onSave={handleSave}
        title={selectedItem ? 'Edit EmailCampaigns' : 'Create EmailCampaigns'}
        fields={formFields}
...
export default EmailCampaignsAdmin;
