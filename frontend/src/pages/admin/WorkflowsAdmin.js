...
function WorkflowsAdmin() {
  ...
  const handleSave = async (formData) => {
    try {
      if (selectedItem) {
        await api.put(`/workflows/${selectedItem.id}`, formData);
  ...
  const handleBulkDelete = async (ids) => {
    try {
      await Promise.all(ids.map((id) => api.delete(`/workflows/${id}`)));
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
      ...
      <AdminFormDialog
        ...
        onSave={handleSave}
        title={selectedItem ? 'Edit Workflows' : 'Create Workflows'}
        fields={formFields}
