...
function MembershipTypesAdmin() {
  ...
  const handleSave = async (formData) => {
    try {
      if (selectedItem) {
        await api.put(`/membership-types/${selectedItem.id}`, formData);
        showSnackbar('Updated successfully');
      } else {
        await api.post('/membership-types', formData);
  ...
  const confirmDelete = async () => {
    try {
      await api.delete(`/membership-types/${deleteId}`);
  ...
  const handleBulkDelete = async (ids) => {
    try {
      await Promise.all(ids.map((id) => api.delete(`/membership-types/${id}`)));
  ...
  const handleExport = async (selectedIds) => {
    try {
      ...
      a.href = url;
      a.download = `membership-types_${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
      ...
      <AdminFormDialog
        ...
        onSave={handleSave}
        title={selectedItem ? 'Edit MembershipTypes' : 'Create MembershipTypes'}
        fields={formFields}
...
export default MembershipTypesAdmin;
