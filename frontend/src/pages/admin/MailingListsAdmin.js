...
function MailingListsAdmin() {
  ...
  const handleSave = async (formData) => {
    try {
      if (selectedItem) {
        await api.put(`/mailing-lists/${selectedItem.id}`, formData);
        showSnackbar('Updated successfully');
      } else {
        await api.post('/mailing-lists', formData);
  ...
  const confirmDelete = async () => {
    try {
      await api.delete(`/mailing-lists/${deleteId}`);
  ...
  const handleBulkDelete = async (ids) => {
    try {
      await Promise.all(ids.map((id) => api.delete(`/mailing-lists/${id}`)));
  ...
  const handleExport = async (selectedIds) => {
    try {
      ...
      a.href = url;
      a.download = `mailing-lists_${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
      ...
      <AdminFormDialog
        ...
        onSave={handleSave}
        title={selectedItem ? 'Edit MailingLists' : 'Create MailingLists'}
        fields={formFields}
...
export default MailingListsAdmin;
