...
function CommitteesAdmin() {
  ...
  const handleSave = async (formData) => {
    try {
      if (selectedItem) {
        await api.put(`/committees/${selectedItem.id}`, formData);
  ...
  const confirmDelete = async () => {
    try {
      await api.delete(`/committees/${deleteId}`);
  ...
  const handleBulkDelete = async (ids) => {
    try {
      await Promise.all(ids.map((id) => api.delete(`/committees/${id}`)));
  ...
  const handleExport = async (selectedIds) => {
    try {
      ...
      a.href = url;
      a.download = `committees_${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
      ...
      <AdminFormDialog
        ...
        onSave={handleSave}
        title={selectedItem ? 'Edit Committees' : 'Create Committees'}
        fields={formFields}
