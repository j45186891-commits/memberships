...
function ForumAdmin() {
  ...
  const handleSave = async (formData) => {
    try {
      if (selectedItem) {
        await api.put(`/forum/categories/${selectedItem.id}`, formData);
  ...
  const confirmDelete = async () => {
    try {
      await api.delete(`/forum/categories/${deleteId}`);
  ...
  const handleBulkDelete = async (ids) => {
    try {
      await Promise.all(ids.map((id) => api.delete(`/forum/categories/${id}`)));
  ...
  const handleExport = async (selectedIds) => {
    try {
      ...
      a.href = url;
      a.download = `forum/categories_${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
      ...
      <AdminFormDialog
        ...
        onSave={handleSave}
        title={selectedItem ? 'Edit Forum' : 'Create Forum'}
        fields={formFields}
